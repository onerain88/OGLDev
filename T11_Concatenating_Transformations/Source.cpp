...
void RenderSceneCB()
{
	...
	Pipeline p;
	p.Scale(sin(scale * 0.1f), sin(scale * 0.1f), sin(scale * 0.1f));
	p.WorldPos(sin(scale), 0.0f, 0.0f);
	p.Rotate(sin(scale) * 90.0f, sin(scale) * 90.0f, sin(scale) * 90);

	glUniformMatrix4fv(glGetUniformLocation(shaderProgram, "_World"), 1, GL_TRUE, (const GLfloat*)p.GetWorldTrans());
